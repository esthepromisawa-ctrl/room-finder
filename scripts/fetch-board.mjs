...
// 予約者名・件名も含めて表示する（利用者の判断）。検索エンジンよけ(noindex/robots)で
// 「たまたま検索で出る」露出は抑える。公開リポジトリのため完全な非公開ではない点は承知のうえ。
...
for (let i = 0; i < DAYS; i++) {
  ...
  days[key] = parseSchedules(await session.getSchedulesRaw())
    .map(x => ({ roomId: x.roomId, start: x.start, end: x.end, title: x.title, owner: x.owner }));
...
console.log(`board.js を生成しました（${DAYS}日分・部屋${rooms.length}）`);
